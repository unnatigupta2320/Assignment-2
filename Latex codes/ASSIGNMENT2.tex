...
\item Now sum of all the angles given and \eqref{eq7} is $360\degree$.So construction of given quadrilateral is \textbf{possible}.
 ...
 \begin{lemma}
\label{lemma}
The vector equation of line passing through point $S$ is given by:
\begin{align}
...
\end{align}
\item Now \eqref{eq8}  can also be written as:
\begin{align}
&\implies \vec{S}=\myvec{I} +\lambda \myvec{\cos\theta \\\sin \theta } \label{eq13}
\end{align}
\end{proof}
\item Now,using \eqref{eq12} we get $\lambda$ as:
\begin{align}
\implies \lambda =6.5
\end{align}
\item Now by putting value of $\lambda$ in \eqref{eq13}  and using \eqref{eq1} we get,
\begin{align}
...
\end{align}
\end{proof}
    \item On putting the values and solving we get:
\begin{align}
